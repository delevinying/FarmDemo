...
{
	[Header("--- Animator of target Character ---")]
    ...
    [MenuItem("3rd Person Controller/Tools/Ragdoll")]
	...
	void RagdollAudioSource()
	{
		if(Selection.activeGameObject!=null)
		{
			...
			var rag = Selection.activeGameObject.AddComponent<Ragdoll>();
	...
	void BuildBodies ()
	{
		foreach (BoneInfo bone in bones)
		{
			...
			bone.anchor.gameObject.AddComponent<CollisionMessage>();
	...
	void CalculateMass ()
	{
        ...
        {
            if (proportionalMass)
                bone.anchor.GetComponent<Rigidbody>().mass = 1;
            else
	...
	void AddHeadCollider ()
	{
		...
		float radius =Vector3.Distance(rightArm.transform.position ,leftArm.transform.position);
		radius /= 4;
